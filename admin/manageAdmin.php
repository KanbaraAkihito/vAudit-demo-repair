...
if (isset($_SESSION['admin'])) {
	...
	if(isset($_POST['username']) && isset($_POST['password']) && isset($_POST['upass'])){
		$clean_name = clean_input($_POST['username']);
		$clean_pass = clean_input($_POST['password']);
		$clean_upass = clean_input($_POST['upass']);
		// 校验旧密码
		$query_p = "select * from admin where admin_pass=SHA('$clean_upass')";
...
<form class="bs-example form-horizontal" action="manageAdmin.php" method="post" name="log">
	...
	<div class="form-group">
        <label for="inputEmail" class="col-lg-2 control-label">你的密码：</label>
        <div class="col-lg-3">
            <input type="text" name="upass" class="form-control" id="inputEmail">
        </div>
	</div>
    <div class="form-group">
